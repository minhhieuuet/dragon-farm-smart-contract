...
abstract contract KittyCoreInterface {
    function cooAddress() public virtual returns(address);
}
    ...
    function mixGenes(uint256 _genes1, uint256 _genes2, uint256 _targetBlock) public returns (uint256) {
        ...
        for(traitPos = 0; traitPos < 48; traitPos++) {
            ...
            if ((traitPos % 4 == 0) && (genes1Array[traitPos] & 1) != (genes2Array[traitPos] & 1)) {
                uint rand = _sliceNumber(randomN, 3, randomIndex);
                randomIndex += 3;
            ...
            if (ascendedTrait > 0) {
                ...
                uint rand = _sliceNumber(randomN, 1, randomIndex);
                randomIndex += 1;
